...
class TestFromRoman:
    @pytest.mark.parametrize('expected,roman_string', [
        (1, 'I'),
        (4, 'IV'),
        (5, 'V'),
        (6, 'VI'),
    ...
    @pytest.mark.parametrize('invalid_roman_string', [
        'abc',
        'not valid'
    ])
...
class TestToRoman:
    @pytest.mark.parametrize('expected,int_value', [
        ('I', 1),
        ('V', 5),
        ('MCMLXXXIX', 1989)
    ])
    ...
    @pytest.mark.parametrize('int_value', [
        0
        - 1,
        -100,
    ])
